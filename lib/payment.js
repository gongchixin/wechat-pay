...
var URLS = {
  ...
  GROUP_REDPACK_SEND: 'https://api.mch.weixin.qq.com/mmpaymkttransfers/sendgroupredpack',
...
Payment.prototype.sendRedPacket = function(order, callback) {
  ...
  var url = URLS.REDPACK_SEND;
  if (order.total_num > 1) {
    url = URLS.GROUP_REDPACK_SEND;
    order.amt_type = 'ALL_RAND';
    requiredData.push('amt_type');
  }
  
  this._signedQuery(url, order, {
    https: true,
...
Payment.prototype._signedQuery = function(url, params, options, callback) {
  ...
  if (url == URLS.REDPACK_SEND || url == URLS.GROUP_REDPACK_SEND) {
